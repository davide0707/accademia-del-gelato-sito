import { createClient } from '@sanity/client';

// Modifica un aggiornamento già pubblicato (titolo/testo/foto), senza
// dover cancellare e ripubblicare da zero. Stessa protezione a PIN delle
// altre funzioni portiere.
const client = createClient({
  projectId: process.env.SANITY_PROJECT_ID,
  dataset: process.env.SANITY_DATASET || 'production',
  apiVersion: '2024-01-01',
  token: process.env.SANITY_WRITE_TOKEN,
  useCdn: false,
});

const TITOLO_MAX = 60;
const TESTO_MAX = 240;
const FOTO_MAX_BYTES = 8 * 1024 * 1024;

export default async function handler(req, res) {
  if (req.method !== 'POST') {
    res.setHeader('Allow', 'POST');
    return res.status(405).json({ errore: 'Metodo non consentito' });
  }

  const { pin, id, titolo, testo, fotoBase64, rimuoviFoto } = req.body || {};

  if (!process.env.ROBERTO_PIN) {
    console.error('ROBERTO_PIN non configurato');
    return res.status(500).json({ errore: 'Configurazione mancante lato server' });
  }
  if (pin !== process.env.ROBERTO_PIN) {
    return res.status(401).json({ errore: 'PIN errato' });
  }

  if (!id || typeof id !== 'string' || !id.trim()) {
    return res.status(400).json({ errore: 'ID mancante' });
  }
  if (!titolo || !testo) {
    return res.status(400).json({ errore: 'Titolo e testo sono obbligatori' });
  }
  if (titolo.trim().length === 0 || titolo.length > TITOLO_MAX) {
    return res.status(400).json({ errore: `Il titolo deve avere tra 1 e ${TITOLO_MAX} caratteri` });
  }
  if (testo.trim().length === 0 || testo.length > TESTO_MAX) {
    return res.status(400).json({ errore: `Il testo deve avere tra 1 e ${TESTO_MAX} caratteri` });
  }

  let fotoAssetRef = null;
  if (fotoBase64) {
    const match = /^data:(image\/(?:jpeg|png|webp));base64,(.+)$/.exec(fotoBase64);
    if (!match) {
      return res.status(400).json({ errore: 'Formato foto non valido (serve JPEG, PNG o WebP)' });
    }
    const [, mimeType, base64Data] = match;
    const buffer = Buffer.from(base64Data, 'base64');
    if (buffer.length > FOTO_MAX_BYTES) {
      return res.status(400).json({ errore: 'Foto troppo pesante' });
    }
    fotoAssetRef = { buffer, mimeType };
  }

  try {
    const patch = client.patch(id.trim()).set({
      titolo: titolo.trim(),
      testo: testo.trim(),
    });

    if (fotoAssetRef) {
      // sostituisce la foto: carica il nuovo asset e aggiorna il riferimento
      const asset = await client.assets.upload('image', fotoAssetRef.buffer, { contentType: fotoAssetRef.mimeType });
      patch.set({ foto: { _type: 'image', asset: { _type: 'reference', _ref: asset._id } } });
    } else if (rimuoviFoto) {
      // rimuove la foto esistente senza sostituirla
      patch.unset(['foto']);
    }
    // se nessuno dei due, la foto esistente resta cosi' com'e'

    const aggiornato = await patch.commit();

    return res.status(200).json({ ok: true, id: aggiornato._id });
  } catch (err) {
    console.error('Errore modifica Sanity:', err);
    return res.status(500).json({ errore: 'Errore durante il salvataggio, riprova' });
  }
}
